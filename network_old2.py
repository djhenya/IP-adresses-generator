...
#	self.HostsWindow = QtGui.QWidget()
	...
	if cur.execute('select * from network').fetchall() == []: counter=1
	    ...
	    for i in cur:
		cur_array.append(i)
	    counter = int(str(cur_array[len(cur_array)-1])[1])+1
...
class MyList(QtCore.QAbstractListModel):
	...
	if not index.isValid():
	    ...
	    return QtCore.QVariant()
	return QtCore.QVariant(QtCore.QString(str(self.hostlist[index.row()][0])))
				...
				for e in range(256):
				 if i>2:
				    ...
				    for r in range(256):
				      for j in range(4):
				        ss[j] = str(s[j])
				      if r!=0 and r!=255: cur.execute('insert into hosts values(?, ?)', (counter,".".join(ss),))
				      s[3-i+3] += 1
				 else:
				  for j in range(4):
				    ss[j] = str(s[j]) 
				  if e!=0 and e!=255: cur.execute('insert into hosts values(?, ?)', (counter,".".join(ss),))
				 s[3-i+2] += 1
			    else:
			     for j in range(4):
				ss[j] = str(s[j])
			     if w!=0 and w!=255: cur.execute('insert into hosts values(?, ?)', (counter,".".join(ss),))
			    s[3-i+1] += 1
			break
			...
			if i>0:
				...
				if i>1:
					...
					for r in range(256):
					  for j in range(4):
					    ss[j] = str(s[j])
					  cur.execute('insert into hosts values(?, ?)', (counter,".".join(ss),))
					  s[3-i+3] += 1
				      else:
					for j in range(4):
					    ss[j] = str(s[j])
